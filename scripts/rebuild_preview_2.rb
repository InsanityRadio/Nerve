...
class Processor < Nerve::Job::Process
	...
	def process_track track
		...
		Dir::chdir($config['export']['directory'])
		...
		if File.exist?(preview_path) or !File.exist?(exported_path) or (track.status != 5 and track.status != 6)
		...
		begin
			...
			if @track.local_path_waveform == nil or File.exist?(@track.local_path_waveform)
...
tracks = Nerve::Model::Track.where(status: [5, 6])
tracks.each { | t |
